...
import { preprocessMessage } from './chatPreprocess';
...
function calculateConfidence(bestScore: number, clean: string, isContextual: boolean): number {
...
function handleAmbiguousTerms(clean: string): ChatResponse | null {
  for (const [term] of Object.entries(Intents.ambiguousTerms)) {
    ...
    if (regex.test(clean) && clean.split(' ').length <= 3) {
      ...
      if (term === "luz") return { text: responses.ambiguousShortQueryLuz, intent: "ambiguous_short_query", confidence: 1.0 };
...
function handleMiniRag(clean: string): ChatResponse | null {
  // Simple RAG for Glossary
  for (const item of electricalGlossary) {
    if (clean.includes(item.term) || item.aliases.some(a => clean.includes(a))) {
  ...
  for (const key of Object.keys(faqKnowledge)) {
    ...
    const matchCount = questionTokens.filter(t => clean.includes(t)).length;
    if (matchCount >= Math.min(questionTokens.length, 2) && clean.length > 5) {
...
export function understandBeforeAnswer(original: string): { response: ChatResponse, understanding: UnderstandingResult } {
  ...
  let confidence = bestScoreObj ? calculateConfidence(bestScoreObj.score, processed.clean, isContextual) : calculateConfidence(0, processed.clean, isContextual);
...
export function routeIntent(
  ...
  let confidence = bestScoreObj ? calculateConfidence(bestScoreObj.score, clean, isContextual) : calculateConfidence(0, clean, isContextual);

  if (intent !== "product_search" && intent !== "product_price" && intent !== "product_spec_question" && intent !== "product_recommendation" && intent !== "multiple_entities" && intent !== "ask_disambiguation" && intent !== "technical_infrared" && intent !== "product_or_technical_infrared" && intent !== "human_support" && intent !== "missing_product_complaint" && intent !== "repair_context_clarification" && intent !== "ambiguous_short_query" && intent !== "product_not_found" && intent !== "attribute_refinement" && intent !== "product_search_with_nearest_match") {
  ...
  if (intent === "attribute_refinement") {
    ...
    const { products } = searchProducts(contextualInput, mergedEntities, 0, chatContext.softContext.category);
    if (products.length > 0) {
      return { text: `Filtrei as opções com essa especificação:`, products, intent: "context_product_recommendation", confidence: 0.85 };
    }
  ...
  if (intent === "product_search_with_nearest_match") {
    ...
    const { products } = searchProducts(requestedProduct, entities);
    return { text: `Não temos '${requestedProduct} ${requestedType}' exato, mas temos estas opções próximas que atendem perfeitamente à maioria das aplicações dessa faixa. Veja as opções:`, products, intent, confidence: 1.0 };
  }
  ...
  const ragResponse = handleMiniRag(clean);
  ...
  if (intent === "correction_wrong_result") return { text: responses.wrongResultCorrection, intent, confidence };
  ...
  if (intent === "show_more_results") {
    if (chatContext.lastSearch) {
      ...
      if (products.length > 0) {
        ...
        if (offset + 3 < total) actions.unshift({ type: "show_more", label: "Ver mais opções" });
  ...
  if (intent === "product_price") {
    if (hasProd) {
      ...
      if (products.length > 0) {
        const actions: ChatAction[] = [{ type: "quote", label: "Adicionar ao orçamento" }, { type: "whatsapp", label: "Consultar valor" }];
        if (total > 3) actions.unshift({ type: "show_more", label: "Ver mais opções" });
  ...
  if (intent === "product_recommendation") {
    ...
    const actions: ChatAction[] = [{ type: "quote", label: "Adicionar ao orçamento" }, { type: "whatsapp", label: "Consultar valor no WhatsApp" }];

    if (clean.includes("quarto") || clean.includes("sala")) return { text: responses.recommendationQuartoSala, products: recProducts, actions: recProducts ? actions : undefined, intent, confidence };
    if (clean.includes("cozinha") || clean.includes("banheiro")) return { text: responses.recommendationCozinhaBanheiro, products: recProducts, actions: recProducts ? actions : undefined, intent, confidence };
    if (clean.includes("externa") || clean.includes("garagem")) return { text: responses.recommendationExterna, products: recProducts, actions: recProducts ? actions : undefined, intent, confidence };
    
    if (recProducts) return { text: "Aqui estão as melhores opções que encontrei:", products: recProducts, actions, intent, confidence };
  ...
  if (intent === "repair_context_clarification" || (isContextual && chatContext.softContext.subject)) {
    const contextualInput = `${clean} ${chatContext.softContext.subject || ''}`;
    const { products } = searchProducts(contextualInput, chatContext.softContext.entities);
    if (products.length > 0) {
      return { text: `Como estávamos falando sobre ${chatContext.softContext.subject || 'isso'}, encontrei estas opções:`, products, intent: "context_product_recommendation", confidence: 0.85 };
    }
